...
if "last_uploaded" not in st.session_state or st.session_state.last_uploaded != uploaded_file:
    ...
    # Track permanent button transformations (rotations, flips, edge buttons)
...
# Base image for slider adjustments is ALWAYS the clean transformed base
img = st.session_state.transformed_base.copy()
original = st.session_state.original.copy()

# 3. TRANSFORM PANEL (Permanent destructive changes apply to base)
...
# 4. ADVANCED EFFECTS PANEL (Permanent destructive filters apply to base)
...
# 5. IMAGE ADJUSTMENTS PANEL (Transient Sliders - Runs dynamically on rerun)
...
# 6. FILTERS (SLIDERS) PANEL (Transient Sliders - Runs dynamically on rerun)
...
# 7. ZOOM PANEL (Transient Sliders - Runs dynamically on rerun)
...
# 9. NATIVE BUFFER DOWNLOAD SYSTEM
buffer = io.BytesIO()
